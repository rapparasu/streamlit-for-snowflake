'''
 ...
 streamlit run bi_and_ml\app.py
...
#setup for story telling (matplotlib):
...
#df_real_estate_appreciation = pd.read_csv("bi_and_ml/data/BR_real_estate_appreciation_Q1_2023.csv")

#when deploying this to streamcloud always work out the path of the file. 
filename = utils.getFullPath("data/BR_real_estate_appreciation_Q1_2023.csv")
print(f"filename: {filename}")
df_real_estate_appreciation = pd.read_csv(filename)


df_real_estate_appreciation['Annual_appreciation'] = round(df_real_estate_appreciation['Annual_appreciation'], 2) *100
...
2. To compare the selected city against 50 cities (**white dots**)
3. To compare the chosen city against **national average** and the data distribution.
4. To extact insights as "An appreciation above national average & price by square meter below average = possible *opportunity*".
   
''')

cities = sorted(list(df_real_estate_appreciation['Location'].unique()))
city_selection = st.selectbox("Select a city", cities)
your_city = city_selection

selected_city = df_real_estate_appreciation.query('Location == @your_city')
other_cities = df_real_estate_appreciation.query('Location != @your_city')

st.dataframe(df_real_estate_appreciation.query('Location == @your_city')[['Location', 'Annual_appreciation', 'BRL_per_squared_meter']],
...
avg_annual_val = df_real_estate_appreciation['Annual_appreciation'].median()
q1_annual_val = np.percentile(df_real_estate_appreciation['Annual_appreciation'], 25)  
q3_annual_val = np.percentile(df_real_estate_appreciation['Annual_appreciation'], 75)   
...
# Showing up position measures:
avg_price_m2 = df_real_estate_appreciation['BRL_per_squared_meter'].median()
q1_price_m2 = np.percentile(df_real_estate_appreciation['BRL_per_squared_meter'], 25)
q3_price_m2 = np.percentile(df_real_estate_appreciation['BRL_per_squared_meter'], 75)
